...
class ObjectDetectionPipeline:
    ...
    def _configure_pipeline(self) -> None:
        ...
        if self.checkpoint_path and self.resume_training:
            self.cfg.resume = True
            self.cfg.load_from = self.checkpoint_path
            self.logger.info(f'Configured to resume training from: {self.checkpoint_path}')
        elif self.checkpoint_path and not self.resume_training:
            self.cfg.load_from = self.checkpoint_path
            self.cfg.resume = False
            self.logger.info(f'Configured to load weights from: {self.checkpoint_path}')
...
def create_model_config(args: argparse.Namespace) -> Dict[str, Any]:
    """
    Create model configuration dictionary with specified depths.
    
    Args:
        depths: List of depths for each stage of the backbone
    ...
    model = dict(
        ...
        init_cfg=dict(
            type='Pretrained', 
            checkpoint="https://download.openmmlab.com/mmclassification/v0/replknet/replknet-31B_in21k-pre_3rdparty_in1k_20221118-54ed5c46.pth",
            prefix='backbone.',
            )),
        neck=dict(
            type='PAFPN',
...
def parse_args() -> argparse.Namespace:
    ...
    # Model configuration
    parser.add_argument('--depths', type=str, default='[4,4,12,4]',
                       help='Model backbone depths as string representation of list (e.g., "[4,4,12,4]")')
    ...
    parser.add_argument('--scheduler_choice', type=str, default="CosineAnnealingLR", choices=['LinearLR', 'MultiStepLR', 'CosineAnnealingLR'],
                       help='Learning rate scheduler choice')
    parser.add_argument('--checkpoint', type=str, default=None,
                       help='Path to the model checkpoint for resuming training or evaluation')
    ...
    # Data configuration
    parser.add_argument('--band_num', type=int, default=4,
...
def main() -> None:
    ...
    # Parse depths from string
    try:
        depths = ast.literal_eval(args.depths)
        assert isinstance(depths, list), "Depths must be a list"
        assert all(isinstance(d, int) for d in depths), "All depths must be integers"
    except (ValueError, SyntaxError, AssertionError) as e:
        raise ValueError(f"Invalid depths format: {args.depths}. Expected list of integers like '[4,4,12,4]'") from e
    ...
    pipeline = ObjectDetectionPipeline(
        ...
        scheduler_choice=args.scheduler_choice if hasattr(args, 'scheduler_choice') else None,
        val_interval=2,
        skip_cfg=True,
