...
class RADCVFNetHead(ATSSHead, FCOSHead):
    ...
    def __init__(self,
        ...
        ######## HOOKS ########
        self.dilated_kernel_size = 3
        self.dilation_rates = [2,3,4]
    ...
    def _init_layers(self) -> None:
        ...
        ######## HOOKS ########
        self.vfnet_reg_refine_dconv = RADC(
            self.feat_channels,
            self.feat_channels,
            self.dilated_kernel_size,
            self.dilation_rates,
        )
        # from dstar conv to dilated conv <-- support for Myriad X
    ...
    def forward_single(self, x: Tensor, scale: Scale, scale_refine: Scale,
        ...
        reg_feat = self.vfnet_reg_refine_dconv(reg_feat) # just apply the RADC on the reg_feat
        ...
        cls_feat = self.vfnet_cls_dconv(cls_feat)
...
class DilatedConvNet(nn.Module):
    ...
    # Create an instance of the DilatedConvNet with desired parameters
    dilated_conv_net = DilatedConvNet(in_channels=3, out_channels=16, kernel_size=3, dilation=2)
    # Forward pass
    output = dilated_conv_net(dummy_input)
    print(output.shape)
    """
    def __init__(self, in_channels, out_channels, kernel_size, padding, dilation):
        super(DilatedConvNet, self).__init__()
        self.dilated_conv = nn.Conv2d(in_channels, out_channels, kernel_size, padding, dilation=dilation)

    def forward(self, x):
        return self.dilated_conv(x)
    
 
class RADC(nn.Module):
    """
    Residual Attention Dilated Convs with multiple dilation rates,
    residual connections, normalization, dropout, and attention mechanisms.
        Create a dummy input tensor with shape (batch_size, in_channels, height, width)
        > dummy_input = torch.randn(1, 30, 64, 64)
        # Create an instance of the ADC model with desired parameters
        > model = RADC(in_channels=30, out_channels=10, kernel_size=3)
        # Forward pass
        > output = model(dummy_input)
        > print(output.shape)
    """
    
    def __init__(self, in_channels, out_channels, kernel_size, dilation_rates=[2, 4, 8]):
        super(RADC, self).__init__()
        ...
        self.last_conv = nn.Conv2d(in_channels, out_channels, kernel_size=1) if in_channels != out_channels else nn.Identity()
